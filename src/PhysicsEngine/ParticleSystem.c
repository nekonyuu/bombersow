...
void particle_Draw(Particle* particle, Config* config)
{
    ...
    glBegin(GL_TRIANGLE_STRIP); // Dessin par triangles assemblés, plus performant pour le GPU
        glVertex2i(particle->x, particle->y + particle->size_y);
        glVertex2i(particle->x + particle->size_x, particle->y + particle->size_y);
        glVertex2i(particle->x + particle->size_x, particle->y);
